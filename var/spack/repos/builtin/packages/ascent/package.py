...
class Ascent(CMakePackage, CudaPackage):
    ...
    def hostconfig(self):
        ...
        if "+vtkh" in spec:
            ...
            if "^vtk-m ^kokkos" in spec:
                if spec["kokkos"].satisfies("cxxstd=11"):
                    cfg.write(cmake_cache_entry("CMAKE_CXX_STANDARD", "11"))
                elif spec["kokkos"].satisfies("cxxstd=14"):
                    cfg.write(cmake_cache_entry("CMAKE_CXX_STANDARD", "14"))
                elif spec["kokkos"].satisfies("cxxstd=17"):
